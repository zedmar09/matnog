...
export const documentRegistrationSchema = z.object({
  ...
  filename: z.string().regex(/\.pdf$/i, "Choose a bundled sample PDF."),
  attachmentNote: z.string().trim().min(8, "Describe the sample attachment in at least eight characters."),
...
export const custodyAcceptanceSchema = z.object({
  trackingReference: z.string().trim().min(5, "Enter the sample tracking reference."),
